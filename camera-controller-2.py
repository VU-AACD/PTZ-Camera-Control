...
c = Camera('192.168.0.87')  # camera IP or hostname
camera_port = 5678 # this is hardcoded into the library for now
...
def disableTextEntries():
    for entry in entry_boxes:
        entry.config(state = 'readonly')
    enableButton['state'] = 'normal'
    disableButton['state'] = 'disabled'

def enableTextEntries():
    for entry in entry_boxes:
        entry.config(state='normal')
    enableButton['state'] = 'disabled'
...
enableButton = Button(root, text='Enable text entry', state='disabled', bg=store_color, command=enableTextEntries)
enableButton.grid(row=18, column=label_column)
disableButton = Button(root, text='Disable', bg=recall_color, command=disableTextEntries)
disableButton.grid(row=18, column=recall_column)
...
tilt_slider = Scale(root, bg=pan_tilt_color, from_=24, to=-24, length=400, command=tilt_this)
tilt_slider.set(0)
